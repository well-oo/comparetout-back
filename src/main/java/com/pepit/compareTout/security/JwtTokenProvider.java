...
public class JwtTokenProvider {
     ...
     */
    @Value("${security.jwt.token.secret-key:VFcmqKATSUWqyNiC6FjQvjJsVXi1ymAA}")
    ...
    public String createToken(String username, List<Role> roles, int days) {

        Claims claims = Jwts.claims().setSubject(username);
        claims.put("auth", roles.stream().map(s -> new SimpleGrantedAuthority(s.getAuthority())).filter(Objects::nonNull).collect(Collectors.toList()));

        Date now = new Date();
        Date validity = new Date(now.getTime() + (days * validityInMilliseconds));
